...
router.post('/', async (req, res) => {
  try {
    const { name, email, department, skills, trainingSchedule, workflowProgress, joinDate } = req.body;
    ...
    const [result] = await pool.promise().query(
      `INSERT INTO freshers (name, email, department, skills, training_schedule, workflow_progress, join_date, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, COALESCE(?, CURRENT_DATE), NOW(), NOW())`,
      [name, email, department, skillsJson, trainingJson, workflowJson, joinDate]
    );
...
router.get('/realtime/:id', async (req, res) => {
  try {
    ...
    // Record a daily login activity entry if not already present for today
    try {
      await pool.promise().query(
        `INSERT INTO login_activity (fresher_id)
         SELECT ? FROM DUAL
         WHERE NOT EXISTS (
           SELECT 1 FROM login_activity 
           WHERE fresher_id = ? AND DATE(login_at) = CURDATE()
         )`,
        [id, id]
      );
    } catch (ignore) {
    ...
    if (fresher.skills) {
      try {
        skills = JSON.parse(fresher.skills);
      } catch (e) {
        skills = fresher.skills.split(',').map(s => s.trim());
      }
    }
    let trainingSchedule = {};
    if (fresher.training_schedule) {
      try {
        trainingSchedule = JSON.parse(fresher.training_schedule);
      } catch (e) {
        trainingSchedule = {};
      }
    }
    ...
    if (fresher.workflow_progress) {
      try {
        workflowProgress = JSON.parse(fresher.workflow_progress);
      } catch (e) {
        workflowProgress = {};
      }
    }
