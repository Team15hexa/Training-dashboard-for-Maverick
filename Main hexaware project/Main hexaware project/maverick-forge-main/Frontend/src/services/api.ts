...
export const authAPI = {
  ...
  async getLoginActivity(fresherId: number) {
    const res = await fetch(`http://localhost:5000/api/auth/login-activity/${fresherId}?days=30`);
    return res.json();
  }
};
...
export const adminAPI = {
  ...
  removeDepartment: async (departmentId: string, options?: { forceDelete?: boolean; reassignTo?: string }) => {
    return apiRequest(`/department/${departmentId}`, {
      method: 'DELETE',
