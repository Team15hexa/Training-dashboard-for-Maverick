...
const RadarChartComponent: React.FC = () => {
  ...
  const fetchDepartmentData = useCallback(async () => {
    try {
      setLoading(true);
      console.log('🔍 Fetching domain data for radar chart...');
      ...
      if (response.success && response.data) {
        ...
        setError('Error loading domain data');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    return (
      <div className={`rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className="flex items-center justify-center h-64">
                      <div className="text-gray-500">Loading domain data...</div>
        </div>
      </div>
    );
  }
    ...
    <div className={`rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
            Domain performance across multiple dimensions
      ...
      <div className="mb-6 space-y-4">
          ...
          <div className="flex-1">
            ...
            <Select onValueChange={handleDepartmentSelect}>
              <SelectTrigger className={`${darkMode ? 'bg-gray-700 border-gray-600 text-gray-200' : 'bg-white border-gray-300'}`}>
                <SelectValue placeholder="Choose domains to display..." />
              </SelectTrigger>
        ...
        {selectedDepartments.length > 0 && (
          <div>
            <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              Selected Domain ({selectedDepartments.length})
            </label>
        ...
        {selectedDepartments.length === 0 && (
          <div className={`text-center py-4 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            No domain selected. Please select domain to view the chart.
          </div>
        )}
      </div>
