...
const ManageFreshers = () => {
  ...
  const [reassignToDepartment, setReassignToDepartment] = useState<string>("");
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.remove-batch-dropdown')) {
    ...
    try {
      ...
      const individualAnalyses = freshersData?.map((fresher: any) => {
        ...
        const insights = [
          ...
          `Batch ranking: ${fresher.department} Batch`,
      ...
      const analysisData = {
        ...
        recommendations: [
          ...
          'Develop batch-specific advanced modules for high performers',
        ...
        insights: [
          ...
          `Top performing batch: ${Object.entries(getDepartmentStats()).sort((a: any, b: any) => b[1] - a[1])[0]?.[0] || 'N/A'}`,
        ...
        trends: [
          ...
          'Batch-specific performance patterns emerging',
  ...
  const handleRemoveDepartmentConfirm = async () => {
    ...
    try {
      const response = await adminAPI.removeDepartment(departmentToRemove.id, {
        forceDelete,
        reassignTo: reassignToDepartment || undefined
      });
      
      if (response.success) {
        const message = response.affectedFreshers > 0 
          ? `Removed ${departmentToRemove.name} batch and ${response.affectedFreshers} fresher(s) were reassigned.`
          : `Removed ${departmentToRemove.name} batch from the system.`;
        ...
        fetchFreshersData(); // Refresh freshers list
      } else {
        setRemoveDepartmentError(response.error || 'Failed to remove batch');
      }
    } catch (error: any) {
      setRemoveDepartmentError(error.message || 'Failed to remove batch. Please try again.');
    } finally {
    ...
    try {
      ...
      if (response.success) {
        toast({
          ...
          description: `Added ${departmentName} batch to the system.`,
          duration: 3000
        });
        ...
        setAddDepartmentError(response.error || 'Failed to add batch');
      }
    } catch (error: any) {
      setAddDepartmentError(error.message || 'Failed to add batch. Please try again.');
    } finally {
  ...
  return (
    <div className={`flex h-screen ${darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-pink-50 via-rose-50 to-red-50'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                    ...
                    <DropdownMenuItem 
                      onClick={() => {
                        if (departments.length > 0) {
                          ...
                          toast({
                            title: "No Domain",
                            description: "No Domain available to remove.",
                            variant: "destructive",
                            duration: 3000
                          });
                        }
                ...
                <div className="relative remove-batch-dropdown">
                  {showRemoveDepartmentDropdown && (
                ...
                <table className="w-full">
                  <thead>
                    <tr className={`border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                      ...
                      <th className={`text-left py-4 px-6 font-semibold ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Domain</th>
      ...
      <Dialog open={addFresherOpen} onOpenChange={setAddFresherOpen}>
        <DialogContent className={darkMode ? 'bg-gray-800 border-gray-700' : ''}>
          ...
          <form onSubmit={handleFresherSubmit} className="space-y-4">
            ...
            <div>
              <Label htmlFor="department" className={darkMode ? 'text-gray-300' : ''}>Batch</Label>
      ...
      <Dialog open={addDepartmentOpen} onOpenChange={setAddDepartmentOpen}>
        <DialogContent className={darkMode ? 'bg-gray-800 border-gray-700' : ''}>
          <DialogHeader>
                            <DialogTitle className={darkMode ? 'text-white' : ''}>Add New Domain</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleDepartmentSubmit} className="space-y-4">
            <div>
                              <Label htmlFor="departmentName" className={darkMode ? 'text-gray-300' : ''}>Domain Name</Label>
              <Input
                id="departmentName"
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={addDepartmentLoading}>
                {addDepartmentLoading ? 'Adding...' : 'Add Domain'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={removeDepartmentOpen} onOpenChange={setRemoveDepartmentOpen}>
        <DialogContent className={darkMode ? 'bg-gray-800 border-gray-700' : ''}>
          <DialogHeader>
                            <DialogTitle className={darkMode ? 'text-white' : ''}>Remove Domain</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className={darkMode ? 'text-gray-300' : 'text-gray-600'}>
                              Are you sure you want to remove the <strong>{departmentToRemove?.name}</strong> domain?
                This action cannot be undone and will affect all freshers assigned to this domain.
            </p>
            ...
            <div className={`p-4 rounded-lg border ${
              ...
              <p className={`text-sm ${darkMode ? 'text-red-300' : 'text-red-700'}`}>
                <strong>Warning:</strong> Removing this domain will:
              </p>
              <ul className={`text-sm mt-2 space-y-1 ${darkMode ? 'text-red-300' : 'text-red-700'}`}>
                                  <li>• Remove the domain from the system</li>
                  <li>• Affect freshers currently assigned to this domain</li>
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="forceDelete" className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                Force delete (will reassign affected freshers)
              </Label>
            </div>
            ...
            {forceDelete && (
              <div className="space-y-2">
                <Label className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  Reassign freshers to:
                </Label>
                <Select value={reassignToDepartment} onValueChange={setReassignToDepartment}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select target batch (or leave empty for 'Unassigned')" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="">Unassigned</SelectItem>
                    {departments
                      .filter(dept => dept.id !== departmentToRemove?.id)
                      .map((dept) => (
                ...
                <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  If no target batch is selected, freshers will be marked as "Unassigned"
                </p>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            >
                              {removeDepartmentLoading ? 'Removing...' : 'Remove Domain'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
